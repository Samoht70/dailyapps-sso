...
- [ ] T083 [US3] Migration et modèle `Invitation` dans `api/functional/users/` : `id` UUID, `email` string(255) requis, `organization_id` FK requis, `organization_role`, `invited_by_id` FK User, `token_hash` **haché — le jeton en clair n'existe que dans le courriel**, `expires_at` de durée configurable, `accepted_at` nullable (usage unique : renseigné, l'invitation ne vaut plus), `timestamps`
